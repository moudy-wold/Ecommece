...
import { Modal, Space, Spin, Table, notification, Input, GetProps } from "antd";
  ...
  const onFinish = async (e: any) => {
    ...
    try {
      if (userRole == "employee") {
        res = await SearchOnUserForEmployee(value);
      } else {
        res = await SearchOnUserForAdmin(value);
      }
      setSearchedData(res.data.customers);
  ...
  return (
    <div className="">
      ...
      <form onSubmit={(e) => onFinish(e)} className="flex items-center w-full ">
        ...
        />
        {/* <Search placeholder="input search text"  onChange={(e) => { setValue(e.target.value);}}  
        ...
        <button
          ...
          className={`${isLoading ? "bg-white" : "bg-[#006496]"
            } bg-[#006496] w-8 h-8 lg:w-12 lg:h-[52px] text-xs text-white flex items-center justify-center rounded-e-md border-[2px] border-[#006496]`}
        >
          {isLoading ? (
            <Space size="small">
              <Spin size="small" />
      ...
      {openSearchData &&
        ...
        >
          <Table scroll={{ x: 750 }} columns={columns} dataSource={dataToShow} />
        </Modal>
      }
      ...
      {openAddService &&
        ...
        >
          {userRole == "employee" ? (
            ...
            <CustomerDetails id={id} setOpenAddService={setOpenAddService} locale={locale} />
          )}
        </Modal>
      }
      {openChangePassword &&
